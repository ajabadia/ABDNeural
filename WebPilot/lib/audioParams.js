 ...
 *
 * Real-units conversion reuses lib/paramValue.js: contract values are
 * normalised 0..1 on the wire and the DSP wants REAL units (Hz, seconds...),
 * the same direction the native ParameterPanel applies.
 */

import { fromNormalized, getDescriptor } from './parameters.js';

/**
 * contractId -> GlobalParams field index (order from neuronikGlobalParamsLayout).
 * Only the fields the pilot UI can actually reach; bpm has no contract
