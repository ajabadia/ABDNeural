...
/**
 * Default screen selection. En el piloto era "la pantalla del piloto"; en la
 * WebUI pasa a ser la lista de ids con la que arranca una pantalla mientras el
 * reparto por pestañas no está escrito (ticket 8.2). Se mantiene el nombre y el
 * contenido del piloto para que este port sea verificable uno a uno.
 */
...
/** Descriptors for a screen, in declaration order. */
export function describePilotControls(ids = PILOT_PARAMETER_IDS) {
  return ids.map(describeControl).filter(Boolean);
}

/** Default state for a set of parameters, taken from the contract (REAL units). */
...
/**
 * Default state in the STORE's state space: normalised 0..1 for every kind.
...
/**
 * Validate a NORMALISED state (the store's state space): every value must sit
 * in 0..1. validateState() below expects real units / integer choice indexes
